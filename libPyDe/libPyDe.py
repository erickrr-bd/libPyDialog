...
class libPyDe:
	"""
	Property that contains an object of the dialog module (pythondialog library).
	"""
	diag = None
	...
	def createMenuDialog(self, text, height, width, menu_height, choices, title):
		"""
		...
		:arg menu_height: Number of entries displayed in the box (which can be scrolled) at a given time.
		:arg choices: Tuple with the menu options.	
		:arg title: Title to display in the box.
		"""
		code_menu, tag_menu = self.diag.menu(text = text, height = height, width = width, menu_height = menu_height, choices = choices, title = title)
		if code_menu == self.diag.OK:
			return tag_menu
		if code_menu == self.diag.CANCEL:
			return 0
	...
	def createRadioListDialog(self, text, height, width, choices, title, action_to_cancel):
		"""
		...
		:arg title: Title to display in the box.
		:arg action_to_cancel: Method that will be called when the cancel option is selected.
		"""
		while True:
	...
	def createCheckListDialog(self, text, height, width, choices, title, action_to_cancel):
		"""
	...
	def createInputBoxDialog(self, text, height, width, init, action_to_cancel):
		"""
	...
	def createPasswordBoxDialog(self, text, height, width, init, insecure, action_to_cancel):
		"""
	...
	def createInputBoxToNumberDialog(self, text, height, width, init, action_to_cancel):
		"""
		...
		while True:
			code_inputbox, tag_inputbox = self.diag.inputbox(text = text, height = height, width = width, init = init)
			if code_inputbox == self.diag.OK:
				if(not validateDataWithRegularExpression(regular_expresion_to_number, tag_inputbox)):
	...
	def createInputBoxToDecimalDialog(self, text, height, width, init, action_to_cancel):
		"""
		...
		action_to_cancel -- Method that will be called when the cancel option is selected.
		"""
		regular_expresion_to_decimal = re_compile(r'^[1-9](\.[0-9]+)?$')
		while True:
			code_inputbox, tag_inputbox = self.diag.inputbox(text = text, height = height, width = width, init = init)
			if code_inputbox == self.diag.OK:
				if(not validateDataWithRegularExpression(regular_expresion_to_decimal, tag_inputbox)):
	...
	def createInputBoxToIPDialog(self, text, height, width, init, action_to_cancel):
		"""
		...
		while True:
			code_inputbox, tag_inputbox = self.diag.inputbox(text = text, height = height, width = width, init = init)
			if code_inputbox == self.diag.OK:
				if(not validateDataWithRegularExpression(regular_expresion_to_ip, tag_inputbox)):
	...
	def createInputBoxToPortDialog(self, text, height, width, init, action_to_cancel):
		"""
		Method that creates a dialog of type inputbox for IP addresses.
		...
		while True:
			code_inputbox, tag_inputbox = self.diag.inputbox(text = text, height = height, width = width, init = init)
			if code_inputbox == self.diag.OK:
				if(not validateDataWithRegularExpression(regular_expresion_to_port, tag_inputbox)):
	...
	def createFolderOrFileNameDialog(self, text, height, width, init, action_to_cancel):
		"""
		...
		while True:
			code_inputbox, tag_inputbox = self.diag.inputbox(text = text, height = height, width = width, init = init)
			if code_inputbox == self.diag.OK:
				if(not self.utils.validateRegularExpression(regular_expresion_to_name_folder_or_file, tag_inputbox)):
	...
	def createTimeDialog(self, text, height, width, hour, minute, action_to_cancel):
		"""
		...
		:arg action_to_cancel: Method that will be called when the cancel option is selected.
		"""
		code_timebox, tag_timebox = self.diag.timebox(text = text, height = height, width = width, hour = hour, minute = minutes, second = 00)
	...
	def createFileDialog(self, filepath, height, width, title, extension_file, action_to_cancel):
		"""
	...
	def createFormDialog(self, text, elements, height, width, title, action_to_cancel):
		"""
	...
	def createYesOrNoDialog(self, text, height, width, title):
		...
		:arg title: Title to display in the box.
		"""
		tag_yes_or_no = self.diag.yesno(text = text, height = height, width = width, title = title)
